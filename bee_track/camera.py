...
class NoCamerasFoundError(CameraException):
    pass
...
def downscale(img, blocksize):
    ...
    maxes = img[:k * blocksize, :l * blocksize].reshape(k, blocksize, l, blocksize).max(
        axis=(-1, -3))  # from https://stackoverflow.com/questions
    return maxes
...
def downscalecolour(img, blocksize):
    k = int(img.shape[0] / blocksize)
    l = int(img.shape[1] / blocksize)
    maxes = img[:k * blocksize, :l * blocksize, :].reshape(k, blocksize, l, blocksize, 3).max(
        axis=(-2, -4))  # from https://stackoverflow.com/questions
    return maxes
...
class Camera(Configurable):
    ...
    def worker(self):
        ...
        while True:
            ...
            if self.test.value:
                if (photo_object['img'] is not None) and (photo_object['record'] is not None):
                    ...
                    if photo_object['record']['flash']:
                        ...
                        y, x = np.unravel_index((photo_object['img'] + np.random.randn(photo_object['img'].shape[0],
                                                                                       photo_object['img'].shape[1]))[
                                                100:-100, 100:-100].argmin(),
                                                photo_object['img'][100:-100, 100:-100].shape)
                        # print(y,x)
            ...
            if self.debug: print(
                'starting to push photo to queue at %s' % (datetime.datetime.now().strftime("%Y%m%d_%H:%M:%S.%f")))
            if self.fastqueue.value:  # note we don't even pass on a colour image!
                ...
                lowres_photo_object = {item: photo_object[item] for item in
                                       ['index', 'record', 'img']}  # just copy across minimal key items
            ...
            if self.savephotos.value:
                if rec is not None:
                    ...
                    filename = 'photo_object_%s_%s_%s_%04i.np' % (
