function convertJSONtoImageURL(data, drawcrosshairs) {
  ...
  for (var i = 0; i < imdata.length; i += 4) {
    //SC: Why skipping 4? function:applies a scaling factor, and sets the alpha channel to fully opaque.
    c = img[row][col] * scale;
    if (c > 255) {
      c = 255;
    } //SC: This checks if the calculated value c is greater than the maximum color value (255 in most image formats). If it is, it's clamped to 255 to prevent overflow.
    imdata[i] = c;
    imdata[i + 1] = c;
    imdata[i + 2] = c;
    ...
    for (var i = 0; i < tdata["contact"].length; i++) {
      ...
      if (tdata["contact"][i]["prediction"] < 3) {
        drawscaledcrosshair(
          ...
          tdata["contact"][i]["x"],
          tdata["contact"][i]["y"],
          Math.round(-40 * (tdata["contact"][i]["prediction"] - 4)),
          0,
          0,
          255
        );
      ...
      if (tdata["contact"][i]["searchmax"] > 20) {
        drawscaleddiagcrosshair(
          imdata,
          width,
          height,
          tdata["contact"][i]["x"],
          tdata["contact"][i]["y"],
          Math.round(tdata["contact"][i]["searchmax"] / 10),
          255,
          255,
          0
        );
...
$("input#imagenum").bind("input", function () {
  image = parseInt($("input#imagenum").val());
  refreshimages();
});
...
$(document).keypress(function (e) {
  //for the short cut key
  if (e.which == 110) {
    $("button#next").trigger("click");
  }
  if (e.which == 78) {
    $("button#next10").trigger("click");
  }
  if (e.which == 108) {
    $("button#last").trigger("click");
  }
  if (e.which == 76) {
    $("button#last10").trigger("click");
  }
  if (e.which == 114) {
  ...
  if (e.which == 97) {
    $("input:radio#" + (cam + 1)).trigger("click");
  }
  if (e.which == 122) {
    $("input:radio#" + (cam - 1)).trigger("click");
  }
...
$("button.refreshimages").click(function () {
  refreshimages();
}); //SC: I could not find this button
...
$("#image2").mousedown(function (e) {
  if (event.which == 2) {
    ...
    url =
      ...
      image +
      "/" +
      Math.round(chosenloc[0]) +
      "/" +
      Math.round(chosenloc[1]) +
      "/" +
      confidence +
      "/" +
      label;
...
$("#image2").click(function (e) {
  ...
  if (shifted) {
    landmarkname = prompt(
      "Enter landmark name (e.g. m03_top, nestfrontleft)",
      ""
    );
    coords = prompt(
      "Enter coords (e.g. 0,2,3.4 or nest, leave as 'skip' to retain current coords)",
      "skip"
    );
  ...
  while ((x1 < 0) | (y1 < 0) | (x2 > 2047) | (y2 > 1535)) {
    ...
    if (x1 < 0) {
      boxsize = boxsize + x1 - 1;
      continue;
    }
    if (y1 < 0) {
      boxsize = boxsize + y1 * 1.3333333 - 1;
      continue;
    }
    if (x2 > 2047) {
      boxsize = boxsize + (2047 - x2) - 1;
      continue;
    }
    if (y2 > 1535) {
      boxsize = boxsize + (1535 - y2) * 1.3333333 - 1;
      continue;
    }
  }
  refreshimages();
...
function drawDots() {
  ...
  if (dot.y * scale + box + 2 > currentimage["photo"].length) {
    dot.y = (currentimage["photo"].length - box - 2) / scale;
  }
  if (dot.y * scale - box - 2 < 0) {
    dot.y = (box + 2) / scale;
  }
  for (
    var i = Math.round(dot.x * scale - box);
    i < dot.x * scale + box;
    i += 1
  ) {
    for (
      var j = Math.round(dot.y * scale - box);
      j < dot.y * scale + box;
      j += 1
    ) {
  ...
  chosenloc = [
    x1 + ((x2 - x1) * brightloc[0]) / 1024 / scale,
    y1 + ((y2 - y1) * brightloc[1]) / 768 / scale,
  ];
